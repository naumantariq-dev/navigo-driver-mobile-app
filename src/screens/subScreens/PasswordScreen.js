...
import { setSecurityFlag, setSecurityKey } from '../../providers-callable/http';
...
export default function PasswordScreen() {
  ...
  const handleDone = async () => {
    if (!passcode) {
      alert('Please enter a passcode to continue');
      return;
    }
    ...
    if (confirmPasscode === '') {
      ...
      if (passcode === confirmPasscode) {
        setSubmitModal(true);
        console.log('Passcodes match:', passcode);
        const eventDataForFlag = {
          email: user.email,
          flag: true,
        };
        const eventDataForKey = {
          email: user.email,
          key:confirmPasscode
        };
        setUser({...user,securityEnabled:true,securityKey:confirmPasscode});
        setSecurityKey(eventDataForKey,token)
        setSecurityFlag(eventDataForFlag, token);
      } else {
        console.log('Passcodes do not match');
        alert('Passcodes do not match, press redo and try again');
      }
